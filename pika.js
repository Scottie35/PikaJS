...
(function($,Win,Doc,PNode,Pos,ProcData,RetType,ContType,Hdrs,TimeOut,Style,Length,ReqAnimFrame){
$.extend($, {
...
$.extend($.fn, {
	...
	fade: function(delay) {
	  ...
	  var go = function() {
	    ...
	    if (+el[Style].opacity > 0) {
	      (Win[ReqAnimFrame] && Win[ReqAnimFrame](go)) || st(go, 16); // Fallback is 1000ms/60fps = 16ms
	    } else {
	    	el[Style].display = 'none';
	    	el[Style].opacity = '';
	    }
	...
	slideUp: function(delay) {
	  ...
	  var go = function() {
	    ...
	    if (-childHt < topnum) {
	      (Win[ReqAnimFrame] && Win[ReqAnimFrame](go)) || st(go, 16);
	    } else {
				$(el).css({
					display: 'none',
					overflow: elOverflow,
	...
	scrollTo: function(opts) {
	  ...
	  var go = function() {
	    ...
	    if (skrol.top != elSkrol) {
	      ...
	      (Win[ReqAnimFrame] && Win[ReqAnimFrame](go)) || setTimeout(go, 16); // Fallback is 1000ms/60fps = 16ms
	    }
	  };
	  go();
	},
